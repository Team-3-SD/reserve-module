...
const opt = {
  credentials: require("amqplib").credentials.plain("team3", "team3"),
};
amqp.connect("amqp://157.230.55.229", opt, function (error0, connection) {
  if (error0) {
    throw error0;
  }
    ...
    channel.consume(
      ...
      function (msg) {
        ...
        order.order.order_details.forEach((product) => {
          sql = `UPDATE dbpurchase.TMPRODUCTO T SET STOCK = T.STOCK - ${+product.quantity} WHERE CPRODUCT = ${
            product.productId
          }`;
          console.log("Connected!");
...
function factoryFactura(order) {
  order.order.factura = {
    ...
    date: getCurrentDate(),
    totalAmount:
      calcualteAmount(order.order.order_details) +
      calculateIGV(order.order.order_details),
  };
